...
// session_start();
 ...
 // check if the user is login
  // $login = new Login();
  // $admin_data = $login->check_login($_SESSION['admin']);
...
$connection = mysqli_connect("localhost","root","","dbfliptrade");
  ...
  //   $post = new Post();
  //   $id = $_SESSION['admin'];
  //   $admin_r = $post->create_post($id, $_POST, $_FILES);
...
if (isset($_GET['s_btn'])) {
  ...
  $querys=mysqli_query($connection,"SELECT * FROM tbl_trade WHERE title LIKE '%$search%'");
}
...
$all=mysqli_query($connection,"SELECT * FROM tbl_trade ORDER BY quantity DESC");
if ($_GET['search']=="") {
   while ($ROW=mysqli_fetch_array($all)) {
      include 'admin_post.php';
}
}else{
  while ($show=mysqli_fetch_array($querys)) {
       include 'search.php';
}
