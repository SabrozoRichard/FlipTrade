...
<?php 
  ...
  $query=mysqli_query($connection,"SELECT * FROM tbl_trade WHERE title='$search'");
}
  ?>
<!DOCTYPE html>
...
$all=mysqli_query($connection,"SELECT * FROM tbl_trade WHERE quantity<=0 AND userid='$id' ORDER BY quantity DESC");
$name_query=mysqli_query($connection,"SELECT * FROM tbl_user WHERE userid='$id'");
$name_fetch=mysqli_fetch_array($name_query);
$buyer_fname=$name_fetch['firstname'];
$buyer_lname=$name_fetch['lastname'];

$trial=mysqli_query($connection,"SELECT * FROM sold_items WHERE buyer_fname='$buyer_fname' AND buyer_lname='$buyer_lname'");
if ($_GET['search']=="") {
   while ($ROW=mysqli_fetch_array($trial)) {
      include 'history_post.php';
}
}else{
